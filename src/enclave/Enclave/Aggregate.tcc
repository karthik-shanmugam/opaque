...
void aggregate_step2_low_cardinality(Verify *verify_set,
  ...
  for (int i = 0; i < num_passes; i++) {
    ...
    // our offset into the set of distinct groups
    // this get initialized later because it depends on whether or not 
    // the first row of this partition matches the last row of the previous partition
    int offset;
    ...
    for (uint32_t j = 0; j < num_rows; j++) {
      ...
      // initialize the offset correctly
      if (j == 0 && boundary_info.grouping_attrs_equal(&cur)) {
        offset = boundary_info.get_offset() - 1;
      } else if (j == 0) {
        offset = boundary_info.get_offset();
      }
